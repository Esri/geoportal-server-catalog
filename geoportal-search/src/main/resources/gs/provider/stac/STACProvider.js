...
(function(){
  ...
  gs.provider.stac.STACProvider = gs.Object.create(gs.provider.Provider,{

    stacUrl: {writable: true, value: null},
    ...
    description: {writable:true,value:function(task) {
      ...
      json = json.trim();
      json = json.replace(/{url}/g,task.val.escXml(this.stacUrl));
    ...
    items: {writable:true,value:function(task) {
      ...
      this.search(task).then(function(res) {
        ...
        // update url
        json = json.replace(/{url}/g,task.val.escXml(self.stacUrl));
    ...
    execute: {writable:true,value:function(task) {
      this.stacUrl = task.baseUrl+"/stac"; 
      ...
      } else if (task.val.endsWith(v,"/stac") || task.val.endsWith(v,"/ogcrecords/")) {
          return this.description(task);
      } else if (task.val.endsWith(v,"/stac/conformance")) {
          return this.conformance(task);
      } else if (task.val.endsWith(v,"/stac/api")) {
          return this.api(task);
      } else if (task.val.endsWith(v,"/stac/collections/metadata/queryables")) {
          return this.queryables(task);
      } else if (task.val.endsWith(v,"/stac/collections")) {
          return this.collections(task);
      } else if (task.val.endsWith(v,"/stac/collections/metadata")) {
      ...
      } else if (task.val.endsWith(v,"/stac/search")) {
          return this.items(task);
      } else {
