        ...
        <Records>
          ...
          <xsl:for-each select="/csw:GetRecordsResponse/csw:SearchResults/csw:Record | /csw:GetRecordsResponse/csw:SearchResults/csw:BriefRecord | /csw:GetRecordByIdResponse/csw:Record | /csw:GetRecordsResponse/csw:SearchResults/csw:SummaryRecord">
            <Record>
              ...
              <xsl:choose>
                <xsl:when test="count(ows:WGS84BoundingBox)>0">
                  <LowerCorner>
                ...
                <xsl:when test="count(ows:BoundingBox)>0">
                  <LowerCorner>
                  ...
                  <MaxX>
                    <xsl:value-of select="normalize-space(substring-before(ows:BoundingBox/ows:UpperCorner, ' '))"/>
                  </MaxX>
                  <MaxY>
                    <xsl:value-of select="normalize-space(substring-after(ows:BoundingBox/ows:UpperCorner, ' '))"/>
                  </MaxY>
                  <MinX>
                    <xsl:value-of select="normalize-space(substring-before(ows:BoundingBox/ows:LowerCorner, ' '))"/>
                  </MinX>
                  <MinY>
                    <xsl:value-of select="normalize-space(substring-after(ows:BoundingBox/ows:LowerCorner, ' '))"/>
                  </MinY>
                </xsl:when>
                <xsl:otherwise>
              ...
              <Types>
                <xsl:for-each select="./dc:type | ./dct:references">
                  <xsl:value-of select="."/>
                  <xsl:text>&#x2714;</xsl:text>
                  <xsl:value-of select="@scheme"/>
                  <xsl:text>&#x2715;</xsl:text>
