...
namespace com.esri.gpt.csw
{
  ...
  public class CswProfile
  {
    ...
    /// with the result.The response is parsed based on the response xslt.
    /// </remarks>
      ...
      try
      {
        ...
        foreach (XmlNode xmlnode in xmlnodes)
        {
          ...
          if ((lowercorner.Length > 0 && uppercorner.Length > 0))
          {
            ...
            parseFlag = Double.TryParse(lowercorner.Substring(0, lowercorner.IndexOf(' ')), NumberStyles.Number, cultureInfo, out parseResult);
            record.BoundingBox.Minx = parseResult;
            parseFlag = Double.TryParse(lowercorner.Substring(lowercorner.IndexOf(' ') + 1), NumberStyles.Number, cultureInfo, out parseResult);
            record.BoundingBox.Miny = parseResult;
            parseFlag = Double.TryParse(uppercorner.Substring(0, uppercorner.IndexOf(' ')), NumberStyles.Number, cultureInfo, out parseResult);
            record.BoundingBox.Maxx = parseResult;
            parseFlag = Double.TryParse(uppercorner.Substring(uppercorner.IndexOf(' ') + 1), NumberStyles.Number, cultureInfo, out parseResult);
            record.BoundingBox.Maxy = parseResult;
            if (parseFlag == false)
            {
          ...
          {
            // I.e. the profile has XML attribute SupportContentTypeQuery == True
    ...
    /// <summary>
    /// Returns service type derived from a url
    /// </summary>
