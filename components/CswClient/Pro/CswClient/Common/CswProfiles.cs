    ...
    public void loadProfilefromConfig(string filename)
    {
      try
      {
        ...
        foreach (XmlNode xmlnode in xmlnodes)
        {
          ...
          if (node != null) {
            displayResponseXslt = node.InnerText.Trim();
          }
          ...
          if (metadataxslt != "")
          {
            metadataxslt = System.IO.Path.Combine(profileDirName, metadataxslt);
          }
          if (displayResponseXslt != "")
          {
            displayResponseXslt = System.IO.Path.Combine(profileDirName, displayResponseXslt);
          }
